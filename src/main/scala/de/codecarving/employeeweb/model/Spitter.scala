...
object Spitter extends Actor with Loggable {
  ...
  private val consumer = new Consumer(Props.get(propsPrefix + "Consumer", ""),
                                      Props.get(propsPrefix + "ConsumerSecret", ""))
  private val token = new Token(Props.get(propsPrefix + "Token", ""),
                                Props.get(propsPrefix + "TokenSecret", ""))

  /**
   * Cutting off unnecessary chars in order to get a nice Tweet
   * length of 140 characters.
   * @param subject The Entry subject.
   * @param tinyurl A tinyurl generated by a URL service.
   * @param semester The Mailinglist identifiers with # infront of them.
   */
